...
- `tests/integration/must-have-parity-responsive.test.ts`
- Component-specific regressions:
    - `packages/vueforge/src/components/__tests__/carousel.test.ts` (swipe)
    - `packages/vueforge/src/components/__tests__/speed-dial.test.ts` (touch-first action flow)
    - `packages/vueforge/src/components/__tests__/tree-table.test.ts` (table structure with overflow wrapper)
    - `packages/vueforge/src/components/__tests__/image.test.ts` (preview overlay surface)
