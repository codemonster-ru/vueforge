...
- The sidebar and workspace share one body layer with rounded top corners above the topbar background.
- A system border along the rounded top edge separates the body layer from the global topbar.
- The sidebar and workspace use the same background to form a continuous administrative surface.
- An inset vertical divider using the standard `VfDivider` tokens separates the sidebar and workspace.
- The footer remains inside the workspace and is pushed to its bottom when content is short.
- At the `lg` breakpoint and below, the sidebar is hidden and the workspace spans the available width.
