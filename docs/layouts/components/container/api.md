...
| --- | --- | --- | --- |
| `as?` | `string` | `div` | Underlying HTML tag name used for rendering. |
| `size?` | `'md' \| 'lg' \| 'xl' \| '2xl'` | — | Size token for spacing and dimensions. |
| `fluid?` | `boolean` | `false` | Removes max-width constraint and uses full available width. |
...
| Name | Parameters | ReturnType | Description |
| --- | --- | --- | --- |
| `default` | `—` | `void` | container content |
