...
| `VfMenuItem` | Split | CmMenu covers action/link selection and basic states through its item collection; retained icons and rich item/link metadata are not fully covered | Resolve the remaining direct-menu gap in `CMUI-179`; do not add a standalone item component unless the collection contract proves insufficient |
