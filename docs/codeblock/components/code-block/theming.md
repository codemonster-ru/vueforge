...
With `theme="inherit"`, the component resolves the nearest `data-theme` or `data-vf-theme` boundary. Explicit `light` and
`dark` values create a local boundary, so nested and scoped themes use the same token contract as the document root.

## Color tokens

| CSS variable                                | Default role                                    | Purpose                               |
| ------------------------------------------- | ----------------------------------------------- | ------------------------------------- |
| `--vf-codeblock-background-color`           | `--vf-color-background-surface-subtle`          | Existing VueForge 1.x editor material |
...
The package keeps the existing `--vf-codeblock-*` variables as its VueForge 1.x customization surface. Their built-in
values are semantic-first and retain legacy fallbacks. `--vf-codeblock-disabled-opacity` remains an active VueForge 1.x
hook, while its built-in value is `1` so the disabled state does not reduce syntax contrast. Existing themes may still
override it deliberately; the disabled copy action itself stays hidden and does not acquire hover/focus treatment.
...
`--vf-codeblock-syntax-ansi-*` variables. The built-in `diff` language uses the inserted/deleted roles. VueForge 1.x does
not expose an API for whole-line highlights or separate diff-line backgrounds; those remain a documented limitation.
...
The legacy `--vf-codeblock-background-color` remains canonical: changing it also changes the syntax and code surfaces.
