...
| `label?`         | `string`                 | —           | Visible label content rendered next to the control.           |
| `static?`        | `boolean`                | `false`     | Disables thumb translation animation in the `switch` variant. |
| `thumbContrast?` | `VfSwitchThumbContrast`  | `auto`      | Thumb contrast strategy against current surface tone.         |
| `buttonVariant?` | `'secondary' \| 'ghost'` | `secondary` | Button appearance used by the `button` variant.               |
...
| `VfSwitchThumbContrast` | `'auto' \| 'inverse'`  |
