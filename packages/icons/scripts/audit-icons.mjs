...
const migrationBatch20Path = resolve(rootDir, 'src/lib/iconMigrationBatch20.json');
const migrationBatch21Path = resolve(rootDir, 'src/lib/iconMigrationBatch21.json');
const migrationBatch22Path = resolve(rootDir, 'src/lib/iconMigrationBatch22.json');
const migrationBatch23Path = resolve(rootDir, 'src/lib/iconMigrationBatch23.json');
const migrationBatch24Path = resolve(rootDir, 'src/lib/iconMigrationBatch24.json');
const migrationBatch25Path = resolve(rootDir, 'src/lib/iconMigrationBatch25.json');
...
const migrationBatch20 = JSON.parse(readFileSync(migrationBatch20Path, 'utf8'));
const migrationBatch21 = JSON.parse(readFileSync(migrationBatch21Path, 'utf8'));
const migrationBatch22 = JSON.parse(readFileSync(migrationBatch22Path, 'utf8'));
const migrationBatch23 = JSON.parse(readFileSync(migrationBatch23Path, 'utf8'));
const migrationBatch24 = JSON.parse(readFileSync(migrationBatch24Path, 'utf8'));
const migrationBatch25 = JSON.parse(readFileSync(migrationBatch25Path, 'utf8'));
...
const result = {
  ...
  migrationBatch20Consistency,
  migrationBatch21Consistency,
  migrationBatch22Consistency,
  migrationBatch23Consistency,
  migrationBatch24Consistency,
  migrationBatch25Consistency,
...
if (Object.values(migrationBatch20Consistency).some((items) => items.length > 0)) {
  throw new Error(`Migration batch 20 mismatch: ${JSON.stringify(migrationBatch20Consistency)}`);
}

if (Object.values(migrationBatch21Consistency).some((items) => items.length > 0)) {
  throw new Error(`Migration batch 21 mismatch: ${JSON.stringify(migrationBatch21Consistency)}`);
}

if (Object.values(migrationBatch22Consistency).some((items) => items.length > 0)) {
  throw new Error(`Migration batch 22 mismatch: ${JSON.stringify(migrationBatch22Consistency)}`);
}

if (Object.values(migrationBatch23Consistency).some((items) => items.length > 0)) {
  throw new Error(`Migration batch 23 mismatch: ${JSON.stringify(migrationBatch23Consistency)}`);
}

if (Object.values(migrationBatch24Consistency).some((items) => items.length > 0)) {
  throw new Error(`Migration batch 24 mismatch: ${JSON.stringify(migrationBatch24Consistency)}`);
}

if (Object.values(migrationBatch25Consistency).some((items) => items.length > 0)) {
  throw new Error(`Migration batch 25 mismatch: ${JSON.stringify(migrationBatch25Consistency)}`);
}

if (Object.values(migrationBatch26Consistency).some((items) => items.length > 0)) {
  throw new Error(`Migration batch 26 mismatch: ${JSON.stringify(migrationBatch26Consistency)}`);
}

if (Object.values(migrationBatch27Consistency).some((items) => items.length > 0)) {
  throw new Error(`Migration batch 27 mismatch: ${JSON.stringify(migrationBatch27Consistency)}`);
}
