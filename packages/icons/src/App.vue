...
import migrationBatch24BeforeJson from './lib/iconMigrationBatch24Before.json';
import migrationBatch24Json from './lib/iconMigrationBatch24.json';
import migrationBatch25BeforeJson from './lib/iconMigrationBatch25Before.json';
import migrationBatch25Json from './lib/iconMigrationBatch25.json';
import migrationBatch26BeforeJson from './lib/iconMigrationBatch26Before.json';
import migrationBatch26Json from './lib/iconMigrationBatch26.json';
import migrationBatch27BeforeJson from './lib/iconMigrationBatch27Before.json';
...
const migrationBatch24Before = migrationBatch24BeforeJson as Partial<Record<IconName, string>>;
const migrationBatch24 = migrationBatch24Json as IconName[];
const migrationBatch25Before = migrationBatch25BeforeJson as Partial<Record<IconName, string>>;
const migrationBatch25 = migrationBatch25Json as IconName[];
const migrationBatch26Before = migrationBatch26BeforeJson as Partial<Record<IconName, string>>;
const migrationBatch26 = migrationBatch26Json as IconName[];
const migrationBatch27Before = migrationBatch27BeforeJson as Partial<Record<IconName, string>>;
...
const viewOptions: Array<{ id: View; label: string }> = [
  ...
  { id: 'batch24', label: 'Batch 24 review' },
  { id: 'batch25', label: 'Batch 25 review' },
...
const reviewFamilyByIcon: Partial<Record<IconName, string>> = {
  ...
  key: 'Security access',
  share: 'Communication actions',
  send: 'Communication actions',
  phone: 'Communication actions',
  cpu: 'System hardware',
  plug: 'System hardware',
  activity: 'Dynamic signals',
...
const activeReviewCopy = computed(() => {
  ...
  if (view.value === 'batch24') {
    return {
      eyebrow: 'Migration batch 24 · approved',
      note: 'Two security-access icons approved by the project owner.',
    };
  }
  if (view.value === 'batch25') {
    return {
      eyebrow: 'Migration batch 25 · approved',
      note: 'Three communication-action icons approved by the project owner.',
    };
  }
  if (view.value === 'batch26') {
    return {
      eyebrow: 'Migration batch 26 · approved',
      note: 'Two system-hardware icons approved by the project owner.',
    };
  }
  if (view.value === 'batch27') {
    return {
      eyebrow: 'Migration batch 27 · approved',
      note: 'Two dynamic-signal icons approved by the project owner.',
    };
  }
  if (view.value === 'batch28') {
    return {
      eyebrow: 'Migration batch 28 · approved',
...
const activeReviewSet = computed(() => {
  ...
  if (view.value === 'batch24') return migrationBatch24;
  if (view.value === 'batch25') return migrationBatch25;
...
const activeBeforeIcons = computed(() => {
  ...
  if (view.value === 'batch24') return migrationBatch24Before;
  if (view.value === 'batch25') return migrationBatch25Before;
...
<style scoped>
/* stylelint-disable no-descending-specificity -- Review modes intentionally layer independent showcase surfaces. */
:global(*) {
