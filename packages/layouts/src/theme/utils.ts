...
export function createLayoutsPreset(preset: VfLayoutPreset) {
  return createThemePresetBase(
    toThemePreset(preset)
  ) as unknown as VfLayoutPreset;
}

export function resolveLayoutsPreset(
  config: VfLayoutThemeConfig = {}
): VfResolvedLayoutPreset {
...
export function resolveLayoutsThemeOptions(
  options: VfLayoutThemeConfig["options"] = {}
) {
...
export function resolveLayoutsThemeConfig(
  config: VfLayoutThemeConfig = {}
): VfResolvedLayoutThemeConfig {
  return {
...
export function layoutsPresetToCssText(config: VfResolvedLayoutThemeConfig) {
  return themePresetToCssTextBase(
    config as unknown as Parameters<typeof themePresetToCssTextBase>[0]
  );
}

export function applyLayoutsThemeConfig(
  config: VfResolvedLayoutThemeConfig,
  targetDocument?: Document
) {
  return applyThemeConfigBase(
    config as unknown as Parameters<typeof applyThemeConfigBase>[0],
    targetDocument
  );
}
