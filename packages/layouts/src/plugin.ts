...
function getMissingCssTokens(tokens: readonly string[]) {
  ...
  return tokens.filter(
    (token) => rootStyles.getPropertyValue(token).trim() === ""
  );
}
...
function isCompositeThemeConfig(
  value: VfVueForgeLayoutsOptions["theme"]
): value is VfVueForgeLayoutsThemeOptions {
  return (
    typeof value === "object" &&
    value !== null &&
    ("core" in value || "layouts" in value)
  );
}
...
export function createVueForgeLayouts() {
  const plugin: Plugin = {
    install(app: App, options: VfVueForgeLayoutsOptions = {}) {
      ...
      const resolvedLayoutsThemeConfig = resolveLayoutsThemeConfig(
        themes.layouts ?? {}
      );
      ...
      if (typeof window !== "undefined" && isDev) {
        const runTokenCheck = () => {
          window.requestAnimationFrame(() => {
            window.requestAnimationFrame(() => {
              warnMissingCssTokensInDev(
                resolvedLayoutsThemeConfig.options.styleId
              );
            });
          });
        };
