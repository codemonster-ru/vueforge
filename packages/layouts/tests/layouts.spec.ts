...
describe("layout theme runtime", () => {
  it("creates a layout preset", () => {
    ...
    expect(preset.tokens.shellSidebarWidth).toBe("19rem");
  });
  ...
  it("applies layout tokens as css variables", () => {
    ...
    expect(style.textContent).toContain(
      "--vf-layout-shell-sidebar-width: 19rem;"
    );
    expect(style.textContent).toContain(
      "--vf-layout-shell-sidebar-width: 17rem;"
    );
    expect(style.textContent).toContain(
      "--vf-layout-header-background: #ffffff;"
    );
    expect(style.textContent).toContain(
      "--vf-layout-header-background: #111827;"
    );
    expect(style.textContent).toContain(
      "--vf-layout-content-subheader-background: var(--vf-layout-header-background);"
    );
    expect(style.textContent).toContain(
      "--vf-layout-content-subheader-border: var(--vf-layout-header-border);"
    );
    expect(style.textContent).toContain("--vf-layout-viewport-height: 100vh;");
    expect(style.textContent).toContain(
      "--vf-layout-document-layout-edge-notch-width: 7px;"
    );
    expect(style.textContent).toContain(
      "--vf-layout-app-shell-header-sticky-z-index: 20;"
    );
    expect(style.textContent).toContain(
      "--vf-layout-error-layout-code-font-size: clamp(2.25rem, 8vw, 5.5rem);"
    );
    expect(style.textContent).toContain(
      "--vf-layout-error-layout-description-color: var(--vf-color-muted);"
    );
  });
  ...
  it("applies theme config through the layouts plugin", () => {
    ...
    expect(style?.textContent).toContain(
      "--vf-layout-shell-sidebar-width: 21rem;"
    );
  });
  ...
  it("applies both core and layout theme config through the layouts plugin", () => {
    ...
    expect(document.querySelectorAll("#vf-layouts-theme-preset")).toHaveLength(
      1
    );
    expect(coreStyle?.textContent).toContain("--vf-color-primary: #ff5a36;");
    expect(coreStyle?.textContent).not.toContain(
      "--vf-layout-shell-sidebar-width:"
    );
    expect(layoutsStyle?.textContent).toContain(
      "--vf-layout-shell-sidebar-width: 22rem;"
    );
  ...
  it("forwards theme mode defaults to VfThemeProvider through the layouts plugin", async () => {
    ...
    expect(document.documentElement.getAttribute("data-test-theme")).toBe(
      "dark"
    );
  });
});
...
describe("document layout", () => {
  ...
  it("adds document container inset only from md and above", async () => {
    ...
    await waitForResponsiveUpdate();
    expect(
      wrapper.find(".vf-document-layout__container").classes()
    ).not.toContain("vf-document-layout__container--padded");
    expect(wrapper.find(".vf-document-layout").classes()).not.toContain(
    ...
    expect(wrapper.find(".vf-document-layout__container").classes()).toContain(
      "vf-document-layout__container--padded"
    );
  });
  ...
  it("supports sidebar and aside column layouts", () => {
    ...
    expect(wrapper.find(".vf-document-layout").classes()).toContain(
      "vf-document-layout--sidebar-content-aside"
    );
    expect(
      wrapper
  ...
  it("switches document layout to compact aside below the runtime xl breakpoint token", async () => {
    ...
    expect(wrapper.find(".vf-document-layout").classes()).toContain(
      "vf-document-layout--compact-aside"
    );
  });
  ...
  it("switches document layout to compact sidebar below the runtime lg breakpoint token", async () => {
    ...
    expect(wrapper.find(".vf-document-layout").classes()).toContain(
      "vf-document-layout--compact-sidebar"
    );
  });
  ...
  it("supports sticky header control for document layout", () => {
    ...
    expect(wrapper.find(".vf-document-layout").classes()).toContain(
      "vf-document-layout--header-sticky"
    );
  });
  ...
  it("renders optional content subheader slot in document layout", () => {
    ...
    expect(
      wrapper.find(".vf-document-layout__content-subheader").exists()
    ).toBe(true);
  });
  ...
  it("supports fill viewport control for document layout", () => {
    ...
    expect(wrapper.find(".vf-document-layout").classes()).toContain(
      "vf-document-layout--fill-viewport"
    );
  });
  ...
  it("supports optional edge notches for document layout", () => {
    ...
    expect(wrapper.find(".vf-document-layout").classes()).toContain(
      "vf-document-layout--edge-notches"
    );
  ...
  it("supports sticky sidebar and aside control for document layout", () => {
    ...
    expect(wrapper.find(".vf-document-layout").classes()).toContain(
      "vf-document-layout--sidebar-sticky"
    );
    expect(wrapper.find(".vf-document-layout").classes()).toContain(
      "vf-document-layout--aside-sticky"
    );
  });
...
describe("error layout", () => {
  it("renders semantic error copy from props", () => {
    ...
    expect(wrapper.find(".vf-error-layout__title").text()).toBe(
      "Page not found"
    );
    expect(wrapper.find(".vf-error-layout__description").text()).toBe(
      "The page you are looking for does not exist."
    );
    expect(wrapper.find(".vf-error-layout__panel").classes()).toContain(
      "vf-error-layout__panel--surface"
    );
  });
  ...
  it("supports slot overrides and optional layout flags", () => {
    ...
    expect(wrapper.find(".vf-error-layout__panel").classes()).not.toContain(
      "vf-error-layout__panel--surface"
    );
