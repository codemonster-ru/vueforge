...
final class CmCheckbox implements ComponentInterface
{
    ...
    public function render(ComponentRenderContext $context): RenderedHtml
    {
        ...
        $classes = (new ClassBuilder())
            ->add('cm-checkbox', "cm-checkbox--{$size}")
            ->addWhen($invalid, 'cm-checkbox--invalid')
            ->add($this->optionalString($attributes->get('class')))
            ->value();

        return RenderedHtml::fromTrustedString(rtrim($this->views->render('components.checkbox', [
            'classes' => $classes,
