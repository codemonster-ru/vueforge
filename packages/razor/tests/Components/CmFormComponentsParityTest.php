...
final class CmFormComponentsParityTest extends TestCase
{
    ...
    public function testMatchesCanonicalSignificantDom(string $slug, string $casePath, string $htmlPath): void
    {
        ...
        foreach ($case['slots'] as $name => $content) {
            $slots[$name] = static fn () => \Codemonster\Razor\Components\RenderedHtml::fromTrustedString($content);
        }
    ...
    {
        foreach (['checkbox', 'radio', 'switch', 'textarea'] as $slug) {
    ...
    private function component(string $slug): CmCheckbox|CmRadio|CmSwitch|CmTextarea
    {
        $views = new RazorEngine(
            new DefaultLocator(dirname(__DIR__, 2) . '/resources/views'),
            cachePath: sys_get_temp_dir() . '/codemonster-ui-form-parity-' . bin2hex(random_bytes(6)),
        );

        return match ($slug) {
            'checkbox' => new CmCheckbox($views),
            'radio' => new CmRadio($views),
            'switch' => new CmSwitch($views),
            'textarea' => new CmTextarea($views),
            default => throw new \InvalidArgumentException("Unknown form component [{$slug}]."),
        };
    }
}
