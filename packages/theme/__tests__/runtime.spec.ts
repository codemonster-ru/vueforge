...
const testPreset = createTestThemePreset({
  name: 'test-preset',
  tokens: {
    colorTextMuted: '#5f6673',
    colorInteractivePrimaryBackground: '#0b63f6',
    colorInteractivePrimarySubtleBackground: 'var(--vf-color-text-muted)',
    colorBackgroundSurface: '#ffffff',
    controlHeightMd: '2.25rem',
  },
  dark: {
    colorTextMuted: '#a0a5ae',
    colorInteractivePrimaryBackground: '#6ea8fe',
  },
});
...
describe('theme runtime', () => {
  ...
  it('serializes theme tokens to css variables', () => {
    const tokensWithDigitSuffixes = {
      colorInteractivePrimaryBackground: '#ff5a36',
    ...
    expect(cssVars['--vf-color-interactive-primary-background']).toBe('#ff5a36');
  ...
  it('resolves light and dark theme tokens from a custom preset', () => {
    const preset = resolveThemePreset({
      ...
      dark: {
        colorInteractivePrimaryBackground: '#ff8f70',
      },
    });

    expect(preset.light.colorInteractivePrimaryBackground).toBe('#ff5a36');
    expect(preset.dark.colorInteractivePrimaryBackground).toBe('#ff8f70');
  ...
  it('builds light and dark css text from a resolved config', () => {
    ...
    expect(cssText).toContain('--vf-color-interactive-primary-background: #ff5a36;');
    expect(cssText).toContain('--vf-color-interactive-primary-background: #ff8f70;');
  ...
  it('emits reversible light and dark variables for nested theme boundaries', () => {
    ...
    expect(cssText).toContain(":is(#theme-root):where([data-brand-theme='light'], [data-vf-theme='light'])");
    expect(cssText).toContain(":where(#theme-root) :where([data-brand-theme='light'], [data-vf-theme='light'])");
    expect(cssText).toContain(":is(#theme-root):where([data-brand-theme='dark'], [data-vf-theme='dark'])");
    expect(cssText).toContain(":where(#theme-root) :where([data-brand-theme='dark'], [data-vf-theme='dark'])");
    ...
    expect(cssText).not.toContain("[data-theme='light']");
    expect(cssText).not.toContain("[data-theme='dark']");
    expect(cssText).toContain("[data-vf-theme='light']");
    expect(cssText).toContain("[data-vf-theme='dark']");
    expect(cssText.match(/--vf-color-interactive-primary-background: #ff5a36;/g)).toHaveLength(2);
    expect(cssText.match(/--vf-color-interactive-primary-background: #ff8f70;/g)).toHaveLength(2);
    expect(cssText.match(/--vf-control-height-md: 2.25rem;/g)).toHaveLength(4);
    expect(
      cssText.match(/--vf-color-interactive-primary-subtle-background: var\(--vf-color-text-muted\);/g),
    ).toHaveLength(4);
    expect(cssText.match(/color-scheme: light;/g)).toHaveLength(2);
    expect(cssText.match(/color-scheme: dark;/g)).toHaveLength(2);
  });

  it('resolves canonical and configured nested mode boundaries in the cascade', () => {
    ...
    themeRoot.dataset.vfTheme = 'dark';
    ...
    expect(getComputedStyle(themeRoot).getPropertyValue('--vf-color-interactive-primary-background')).toBe('#ff8f70');
    expect(getComputedStyle(lightBoundary).getPropertyValue('--vf-color-interactive-primary-background')).toBe(
      '#ff5a36',
    );
    expect(getComputedStyle(darkBoundary).getPropertyValue('--vf-color-interactive-primary-background')).toBe(
      '#ff8f70',
    );
    ...
    expect(getComputedStyle(themeRoot).getPropertyValue('--vf-color-text-muted')).toBe('#a0a5ae');
    expect(getComputedStyle(lightBoundary).getPropertyValue('--vf-color-text-muted')).toBe('#5f6673');
    expect(getComputedStyle(darkBoundary).getPropertyValue('--vf-color-text-muted')).toBe('#a0a5ae');

    themeRoot.dataset.vfTheme = 'light';

    expect(getComputedStyle(themeRoot).getPropertyValue('--vf-color-interactive-primary-background')).toBe('#ff5a36');
    expect(getComputedStyle(themeRoot).colorScheme).toBe('light');
    expect(getComputedStyle(themeRoot).getPropertyValue('--vf-color-text-muted')).toBe('#5f6673');
  ...
  it('lets an explicit light boundary override a custom root dark selector', () => {
    ...
    applyThemeConfig(
      resolveThemeConfig({
        preset: testPreset,
        light: { colorInteractivePrimaryBackground: '#ff5a36' },
        dark: { colorInteractivePrimaryBackground: '#ff8f70' },
        options: {
          rootSelector: '#theme-root',
  ...
  it('builds combined css text from multiple resolved configs', () => {
    ...
    expect(cssText).toContain('--vf-color-interactive-primary-background: #ff5a36;');
    expect(cssText).toContain('--vf-layouts-color-background-surface: #f5f7fb;');
  });
  ...
  it('injects a style tag with resolved theme variables', () => {
    ...
    expect(style.textContent).toContain('--vf-color-interactive-primary-background: #ff5a36;');
  ...
  it('injects multiple style tags grouped by style id', () => {
    ...
    expect(document.getElementById('vf-test-theme')?.textContent).toContain(
      '--vf-color-interactive-primary-background: #ff5a36;',
    );
    expect(document.getElementById('vf-test-theme')?.textContent).toContain(
      '--vf-color-interactive-primary-background: #ff8f70;',
    );
    expect(document.getElementById('vf-test-theme-2')?.textContent).toContain(
      '--vf-layouts-color-background-surface: #f5f7fb;',
    );
  });
});
